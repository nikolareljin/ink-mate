...
InkMate initially targets the battery-equipped, non-touch variant. The supplied listing capture explicitly shows `Color: With Bat No Touch`; its generic “Touch Options” title covers four variants and does not mean this selected unit has touch input. The supplied PDF is a saved product listing, not a revision-specific schematic.

## Evidence labels

- **Listing fact:** directly present in the supplied product text.
...
The ES8311 codec is a working assumption based on related board materials, not a fact established by the supplied listing. Battery capacity, charging limits, battery ADC circuit, panel driver, and expansion pinout are unknown.
...
No complete revision-specific pinout is established by the supplied listing or PDF. GPIOs must remain unassigned/disabled until traced to a reliable schematic or confirmed against vendor firmware and the physical board. Never copy a touch-model pinout or assume V1 and V2 match.
